	...
	if (pFbxRootNode)
	{
		...
		for (int i = 0; i < pFbxRootNode->GetChildCount(); i++)
		{
			...
			for (int l = 0; l < controlCount; l++)
			{
				...
				vertex.point = XMFLOAT3((float)pVertices[l].mData[0] / 2.01, 
										(float)pVertices[l].mData[1] / 2.01, 
										((float)pVertices[l].mData[2] + 1.1) / 2.01);
				vertex.color = XMFLOAT4(1.0, 0.0, 0.0, 1);
