...
namespace Transmission {
	...
	typedef struct
	{
		...
		int tanIndex1, tanIndex2, tanIndex3;
	}FaceType;
	...
	HRESULT TRKLoader::loadTRKFile(char* filePath, VertexBuffer** vBuf, IndexBuffer** iBuf, Renderer* renderer) {
		bool result;
		int vertexCount, textureCount, normalCount, tangentCount, faceCount;
		...
		result = ReadFileCounts(filePath, vertexCount, textureCount, normalCount, tangentCount, faceCount);
		if (!result)
		{
			return E_FAIL;
		}
		...
		result = LoadDataStructures(filePath, vertexCount, textureCount, normalCount, tangentCount, faceCount, vBuf, iBuf, renderer);
		if (!result)
		{
			return E_FAIL;
		}
	...
	bool TRKLoader::ReadFileCounts(char* filename, int& vertexCount, int& textureCount, int& normalCount, int& tangentCount, int& faceCount)
	{
		ifstream fin;
		char input;
	...
	bool TRKLoader::LoadDataStructures(char* filename, int vertexCount, int textureCount, int normalCount, int tangentCount, int faceCount, VertexBuffer** vBuf, IndexBuffer** iBuf, Renderer* renderer)
	{
		vertType* vertices;
		Vertex* vertexBufferArray;
		texType* texcoords;
		normalType* normals;
		tangentType* tangents;
		FaceType* faces;
		ifstream fin;
		int vertexIndex, texcoordIndex, normalIndex, tangentIndex, faceIndex, vertexBufferIndex;
		char input, input2;
		...
		tangents = new tangentType[tangentCount];
		if (!tangents)
		{
			return false;
		}
		...
		while (!fin.eof())
		{
			if (input == 'v')
			{
				...
				// Read in the tangents.
				if (input == 'x')
				{
					fin >> tangents[tangentIndex].x >> tangents[tangentIndex].y >> tangents[tangentIndex].z;

					// Invert the Z normal to change to left hand system.
					tangents[tangentIndex].z = tangents[tangentIndex].z * -1.0f;
					tangentIndex++;
				}
			}
			...
			if (input == 'f')
			{
				...
				if (input == ' ')
				{
					...
					fin >> faces[faceIndex].vIndex3 >> input2 >> faces[faceIndex].tIndex3 >> input2 >> faces[faceIndex].nIndex3 >> input2 >> faces[faceIndex].tanIndex3
						>> faces[faceIndex].vIndex2 >> input2 >> faces[faceIndex].tIndex2 >> input2 >> faces[faceIndex].nIndex2 >> input2 >> faces[faceIndex].tanIndex2
						>> faces[faceIndex].vIndex1 >> input2 >> faces[faceIndex].tIndex1 >> input2 >> faces[faceIndex].nIndex1 >> input2 >> faces[faceIndex].tanIndex1;
					faceIndex++;
		...
		for (int i = 0; i < faceIndex; i++)
		{
			...
			vertexBufferArray[vertexBufferIndex].tangent[0] = tangents[faces[i].tanIndex1 - 1].x;
			vertexBufferArray[vertexBufferIndex].tangent[1] = tangents[faces[i].tanIndex1 - 1].y;
			vertexBufferArray[vertexBufferIndex].tangent[2] = tangents[faces[i].tanIndex1 - 1].z;
			...
			vertexBufferArray[vertexBufferIndex].tangent[0] = tangents[faces[i].tanIndex2 - 1].x;
			vertexBufferArray[vertexBufferIndex].tangent[1] = tangents[faces[i].tanIndex2 - 1].y;
			vertexBufferArray[vertexBufferIndex].tangent[2] = tangents[faces[i].tanIndex2 - 1].z;
			...
			vertexBufferArray[vertexBufferIndex].tangent[0] = tangents[faces[i].tanIndex3 - 1].x;
			vertexBufferArray[vertexBufferIndex].tangent[1] = tangents[faces[i].tanIndex3 - 1].y;
			vertexBufferArray[vertexBufferIndex].tangent[2] = tangents[faces[i].tanIndex3 - 1].z;
